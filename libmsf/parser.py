...
class MsfParser:
    ...
    def _parse_xml_string(self, string: str) -> Union[None, MsfData]:
        try:
            ...
            assert (
                root.tag == "MetasploitV5"
            ), "This is not Metasploit exported xml file"
                ...
                for host in hosts.iter("host"):
                    ...
                    for notes in host.iter("notes"):
                        for note in notes.iter("note"):
                            ...
                            msf_note.port = self._get_port_by_service_id(
                                service_id=msf_note.service_id, services=msf_services
                            )
                    ...
                    for vulns in host.iter("vulns"):
                        for vuln in vulns.iter("vuln"):
                            ...
                            msf_vuln.port = self._get_port_by_service_id(
                                service_id=msf_vuln.service_id, services=msf_services
                            )
                            msf_vuln.refs = list()
...
def main() -> None:
    ...
    parser.add_argument(
        "-f", "--file", type=str, help="set MSF exported file", required=True
    )
